...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params)
{
   ...
   //fix the genesis block
    int nHeightFirstTemp = nHeightFirst;
    if(pindexLast->nHeight == 99)
        nHeightFirstTemp = nHeightFirst + 1; 
    else
        assert(nHeightFirst >= 0);
    const CBlockIndex* pindexFirst = pindexLast->GetAncestor(nHeightFirstTemp);
