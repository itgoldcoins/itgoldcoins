...
void SignVerifyMessageDialog::on_signMessageButton_SM_clicked()
{
    ...
    CItgoldcoinsAddress addr(ui->addressIn_SM->text().toStdString());
    if (!addr.IsValid())
    {
...
{
    CItgoldcoinsAddress addr(ui->addressIn_VM->text().toStdString());
    if (!addr.IsValid())
    {
    ...
    if (!(CItgoldcoinsAddress(pubkey.GetID()) == addr))
    {
        ui->statusLabel_VM->setStyleSheet("QLabel { color: red; }");
