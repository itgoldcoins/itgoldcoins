...
###   This script attempts to download the signature file SHA256SUMS.asc from itgoldcoins.org
...
WORKINGDIR="/tmp/itgoldcoins"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://itgoldcoins.org/bin/"
VERSIONPREFIX="itgoldcoins-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'itgoldcoins-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Itgoldcoins Core binary release signing key installed?"
   fi
